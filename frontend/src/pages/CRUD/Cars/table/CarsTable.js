...
import * as dataFormat from 'pages/CRUD/Cars/table/CarsDataFormatters';

import actions from 'actions/cars/carsListActions';
...
const CarsTable = () => {
  ...
  const count = useSelector((store) => store.cars.list.count);
  const modalOpen = useSelector((store) => store.cars.list.modalOpen);
  const rows = useSelector((store) => store.cars.list.rows);
  const idToDelete = useSelector((store) => store.cars.list.idToDelete);
  ...
  return (
    <div>
      <Widget title={<h4>Cars</h4>} disableWidgetMenu>
        ...
        >
          <DataGrid
            ...
            loading={loading}
            onRowClick={(e) => {
              history.push(`/admin/cars/${e.id}/edit`);
            }}
            autoHeight
          />
        </div>
          ...
          <LinkMaterial
            ...
            href={
              process.env.NODE_ENV === 'production'
                ? window.location.origin + '/api-docs/#/Cars'
                : 'http://localhost:8080/api-docs/#/Cars'
            }
          >
            API documentation for cars
          </LinkMaterial>
        </div>
      </Widget>
