...
import { customerInputSchema, customerSchema } from '../entity/Customer';

export const tag = 'Customer';

export default async (app: FastifyInstance) => {
    const schema = TypeBox.Type.Object({
        q: TypeBox.Type.Optional(TypeBox.Type.Partial(customerSchema, { description: 'Filter query', additionalProperties: false })),
    ...
    app.get<{ Querystring: TypeBox.Static<typeof schema> }>('/customer', {
        // @ts-ignore
        ...
        // @ts-ignore
        const [items, count] = await getCustomRepository(CustomerRepository).filter(req.query, req.query.page, req.query.limit, req.query.field, req.query.sort.toUpperCase());
        return {
            rows: items,
            count,
    ...
    app.get<{
        ...
        schema: {
            ...
            tags: [tag],
            summary: 'Find customer instance to link as relations',
            response: {
    ...
    app.post<{
        ...
        Reply: TypeBox.Static<typeof customerSchema>
    }>('/customer', {
        // @ts-ignore
        preHandler: app.auth([app.verifyAuthorized]),
        schema: {
    ...
    // @ts-ignore
        ...
        const entity = await getCustomRepository(CustomerRepository).findOne({
            where: {
                id: req.params.id,
            },
        ...
        return entity ? entity : reply.notFound('Customer not found');
    });

    const putSchema = TypeBox.Type.Object({
        id: TypeBox.Type.String({ format: 'uuid' }),
        data: TypeBox.Type.Partial(customerInputSchema),
    });
    app.put<{
        Params: TypeBox.Static<typeof find>,
    ...
    app.delete<{
        ...
        const entity = await getCustomRepository(CustomerRepository).softDelete(req.params.id);
        if (!entity.affected) {
            reply.notFound('customer not found');
            return;
        }
